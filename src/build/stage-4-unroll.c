...
void unroll_func_start(Unroll* ur, Scope* scope, Array* unroll) {
    ...
    if(func->gc_decl_count > 0) {
        ...
        // Value *ms = vgen_memset(alc, func->v_cache_stack_pos, vgen_int(alc, func->gc_decl_count * b->ptr_size * 2, type_cache_uint(b)), vgen_int(alc, 0, type_cache_u8(b)));
