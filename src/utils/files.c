...
bool get_fullpath(char *filepath, char *buf) {
    char *fullpath = NULL;
#ifdef WIN32
    char *res = _fullpath(buf, filepath, VALK_PATH_MAX);
#else
...
char *get_binary_dir() {
    ...
    char *buf = malloc(VALK_PATH_MAX);
    strcpy(buf, "");
#if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) || defined(__NT__)
    GetModuleFileName(NULL, buf, VALK_PATH_MAX);
#elif defined(__APPLE__)
    uint32_t size = VALK_PATH_MAX;
    if (_NSGetExecutablePath(buf, &size) < 0) {
        fprintf(stderr, "Could not determine macos valk executable path\n");
        exit(1);
    }
    char *full = malloc(VALK_PATH_MAX);
    get_fullpath(buf, full);
    free(buf);
    buf = full;
#else
    int len = readlink("/proc/self/exe", buf, VALK_PATH_MAX);
    buf[len] = '\0';
#endif
...
char *get_storage_path() {
    ...
    g_storage_path = malloc(VALK_PATH_MAX);
    char *homedir;

#if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) || defined(__NT__)
    homedir = getenv("USERPROFILE");
    const char *validir = "\\.valk";
#else
    homedir = getenv("HOME");
    const char *validir = "/.valk";
#endif
