...
char* ir_value(IR* ir, Value* v) {
    ...
    if (vt == v_func_call) {
        ...
        if (errh) {
            return ir_func_err_handler(ir, errh, call, false);
        }
        return call;
    }
    if (vt == v_gc_buffer) {
    ...
    if (vt == v_ptr_of) {
        return ir_assign_value(ir, v->item);
    }
    if (vt == v_stack) {
        Value* val = v->item;
    ...
    if (vt == v_op) {
        ...
        return ir_op(ir, op, left, right, v->rett);
    }
    if (vt == v_compare) {
        VOp *vop = v->item;
        int op = vop->op;
        char *l = ir_value(ir, vop->left);
        char *r = ir_value(ir, vop->right);
