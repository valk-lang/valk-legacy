...
function get_valk_type(string $type, array $unions): string
{
    if($type[0] == '[') {
        $split = explode(' x ', trim($type, "[]"));
        return 'inline [' . get_valk_type($split[1], $unions) . ', ' . $split[0] . ']';
    }
    if (strpos($type, '%struct.') === 0) return str_replace('%struct.', 'inline libc_', $type);
    if (strpos($type, '%union.') === 0) {
        $uname = str_replace('%union.', '', $type);
        return get_valk_type($unions[$uname], $unions);
    } 
    return $type;
}
...
function gen_valk_structs(string $ir, array $vars): string
{
    ...
    foreach($mm[1] as $k => $name) {
        ...
        if(!$struct_name) {
            // Inline type
            $structs[$name] = ['types' => [$type], 'valk_name' => $name, 'packed' => false];
            $struct_name = $name;
        }
        ...
        if(isset($vars[$name]['fields'])) {
            ...
            $structs[$struct_name]['valk_name'] = $name;
    ...
    foreach($structs as $cname => $s) {
        $valk_name = 'libc_' . ($s['valk_name'] ?? $cname);
        $code .= "\nstruct " . $valk_name  . ($s['packed'] ? ' packed' : '') . " {\n";
        foreach($s['types'] as $i => $type) {
            $names = $s['fields'] ?? [];
            $fn = $names[$i] ?? ('prop_' . $i);
            $vtype = get_valk_type($type, $unions);
            $code .= "    $fn: $vtype\n";
        }
        $code .= "}\n";
    }
